...
def get_args():
    ...
    parser.add_argument('--input_dim', default=2*200, type=int,
                        help='dimensionality of input tensor')
    parser.add_argument('--hidden_dim', default=400,
                        type=int, help='hidden dimension of mlp')
    parser.add_argument('--learn_rate', default=1e-5, #1e-3, 5e-4, 1e-4, 5e-5, 1e-5
...
class KSOrbitalDataset(Dataset):
    def __init__(self, csv_file, csv_file2, root_dir, train=True, transform=None):
        ...
        q = np.real(self.orbital_coefficients)
        p = np.imag(self.orbital_coefficients) 
        dqdt = np.real(self.orbital_coefficients_dt)
        dpdt = np.imag(self.orbital_coefficients_dt)
...
def train(args):
    # loading dataset
    train_dataset = KSOrbitalDataset(csv_file='2e_wf.csv',
                                     csv_file2='2e_wf_dt.csv',
                                     root_dir='./data',
                                     train=True)

    test_dataset = KSOrbitalDataset(csv_file='2e_wf.csv',
                                    csv_file2='2e_wf_dt.csv',
                                    root_dir='./data',
                                    train=False)
    ...
    optim = torch.optim.Adam(
        model.parameters(), args.learn_rate, weight_decay=0)
    ...
    for epoch in range(num_epochs):
        for (x, dxdt) in train_loader:
            ...
            # dHdt = model.hamiltonian_time_derivative(x)
            # zero_matrix = torch.zeros_like(dHdt)
            loss = loss_func(dxdt, dxdt_hat)  # + loss_func(dHdt, zero_matrix)
            loss.backward()
